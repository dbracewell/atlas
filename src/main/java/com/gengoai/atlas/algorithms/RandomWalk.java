...
package com.gengoai.atlas.algorithms;
...
public class RandomWalk<V> implements VertexTraversal<V>, VertexWalker<V>, Serializable {
  ...
  public Iterator<V> iterator(V startingVertex) {
    return new RandomWalkIterator<>(graph, Preconditions.checkNotNull(startingVertex), isWeighted);
  }

  @Override
  public V walk(V startingVertex, int numberOfSteps) {
    Preconditions.checkNotNull(startingVertex);
    Preconditions.checkArgument(numberOfSteps >= 0, "Number of steps must be >= 0.");
  ...
  protected static class RandomWalkIterator<V> implements Iterator<V> {
    ...
    private void getNext() {
      if (graph.outDegree(currentVertex) == 0) {
        ...
        currentVertex = Iterables.get(graph.getSuccessors(currentVertex), random.nextInt(graph.outDegree(currentVertex)));
      } else {
